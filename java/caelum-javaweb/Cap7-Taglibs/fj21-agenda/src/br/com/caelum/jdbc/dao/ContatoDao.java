...
public class ContatoDao {
	...
	public void adiciona(Contato contato) {
		 ...
		 * 
		 * sqlId = " select seq_contatos.currval from dual "; } else
		 */
		if (this.connection instanceof com.mysql.jdbc.JDBC4Connection) {
			sqlInsert = " insert into contatos "
		...
		try {
			...
			// Há um método melhor, independente de SGBD sendo usado na classe
			// FuncionarioDao, método adiciona()
			if (sqlId != null) {
	...
	public Contato pesquisar(long id) {
		...
		try {
			...
			try {
				...
				// O Access não suporta Long, então tive que fazer esta
				// "adaptação"
	...
	public void altera(Contato contato) {
		String sql = " update contatos "
				+ " set nome = ?, email = ?, endereco = ?, dataNascimento = ?, telefone = ? "
				+ " where id = ? ";
		...
		try {
			...
			try {
				...
				// O Access não suporta Long, então tive que fazer esta
				// "adaptação"
				stmt.setInt(6, contato.getId().intValue());
			}
			stmt.execute();
		} catch (SQLException e) {
			throw new DaoException(e);
