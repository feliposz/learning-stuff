...
main(int argc, char *argv[])
{
    ...
    float resize;
    char dummy;
    if (sscanf(argv[1], " %f %c", &resize, &dummy) != 1 || resize < 0.0 || resize > 100.0) {
        printf("Resize factor must be a number between 0 and 100.\n");
        return 1;
    }
    char *infile = argv[2];
    char *outfile = argv[3];
    ...
    for (int i = 0, biHeight = abs(bi.biHeight); i < biHeight; i++)
    {
        ...
        {
            // adjust i, j and biWidth to the original size and get the color triple from the original image
            int lineOffset = (int)(i / resize) * originalWidth;
            int columnOffset = (int)(j / resize);
            RGBTRIPLE *triple = image + lineOffset + columnOffset;
