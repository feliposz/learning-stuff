...
Shuttle.prototype.updatePosition = function(dt) 
{
    ...
    // use fuel
    this.fuel -= (Math.abs(forward) + Math.abs(strafe)) / 25;
    if (this.fuel < 0) {
        this.fuel = 0;
    }
    ...
    // check if touching any gas station
    for (var station in STATIONS) {
        if (this.distance(STATIONS[station].lat, STATIONS[station].lng) < 30) {
            //refuel
            this.fuel = FULLGASTANK;
        }
    }
}
